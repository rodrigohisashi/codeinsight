...
class mod_vpl_submission_CE extends mod_vpl_submission {
    ...
    public function retrieveresult($processid) {
        ...
        if ($response['interactive'] == 0 && $processinfo->type == 2) {
            ...
            $submitted_files = $this->get_submitted_fgm()->getFileList();
            $student_code = "";
            foreach ($submitted_files as $file) {
                $student_code .= $this->get_submitted_fgm()->getFileData($file) . "\n";
            }

            $exercise_description = $this->vpl->get_instance()->intro;
            $execution_result = $response['execution']; 
            
            // Configuração da URL e dos dados para a API.
            $url = 'http://flask-app:5100/generate_feedback';
            $postData = [
                'problem_description' => $exercise_description,
                'code_snippet' => $student_code,
                'execution_code' => $execution_result
            ];
                // Enviar os dados para a API usando cURL.
            $ch = curl_init($url);
            ...
            // Adicione o feedback da API à resposta.
            $apiResponse = json_decode($result, true);
            $response['execution'] .= "\n\nFeedback Gerado por IA (Pode conter erros): " . $apiResponse['feedback'];
            file_put_contents("feedback.txt", $apiResponse['feedback']);
